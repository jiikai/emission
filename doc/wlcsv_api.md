...
Its main goal is to ease the association of multiple callbacks to column, row, string or regex match in a given field of the csv data.
...
wlcsv includes the following standard headers:
...
#include <string.h>
```
Additionally, it includes the PCRE regular expression library header and two locally present
headers for the csv parser and hash table functionality.
```c
#include <pcre.h>
#include "csv.h"
#include "uthash.h"
...
- Define the version number of this code.
```c
...
- PCRE substring buffer default size in bytes.
Change at compile-time by passing `-DWLCSV_PCRE_OVECCOUNT=<value>` to the compiler.
```c
#define WLCSV_PCRE_OVECCOUNT 0xFF
```
...
#### `WLCSV_STATE_MEMBER_GET(stt, member)`

A `NULL`-safe, generic convenience "getter" for [the state structure](#wlcsv_state_st).

```c
#define WLCSV_STATE_MEMBER_GET(stt, member) (stt ? stt->member : UINT_MAX)
```
...
__See:__ [`wlcsv_init()`](#wlcsv_init-)
...
__See:__ [`wlcsv_state_get()`](#wlcsv_state_get-), [`wlcsv_init()`](#wlcsv_init-)
...
__See:__ [`wlcsv_callbacks_set`](#wlcsv_callbacks_set-)
...
```
__See:__ [`wlcsv_callbacks_eor_set()`](#wlcsv_callbacks_eor_set-)
...
__See:__ [`wlcsv_file_path()`](#wlcsv_file_path-), [`wlcsv_file_preview()`](#wlcsv_file_preview-)
...
```c
unsigned wlcsv_callbacks_set(wlcsv_ctx_st *ctx,
...
__See:__ [`wlcsv_callbacks_clear()`](#wlcsv_callbacks_clear-)
...
- Fails silently if `ctx` is `NULL`.

__See:__ [`wlcsv_state_get()`](#wlcsv_state_get-)
